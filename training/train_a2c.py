"""Training utilities for the Stable-Baselines3 A2C baseline."""
...
from algorithms.sb3_a2c import build_a2c_model, make_a2c_env
from config import (
...
def run_a2c_training(
    *,
    ...
    total_timesteps: int = DEFAULT_A2C_TIMESTEPS,
    ...
    """Train an SB3 A2C agent and save its artifacts."""
    ...
    try:
        ...
        raise ImportError(
            "Stable-Baselines3 is required for A2C training. "
            "Install the dependencies in requirements.txt."
        ) from exc

    set_global_seeds(seed)

    model_dir = SAVED_MODELS_DIR / "a2c"
    output_dir = Path(results_dir) if results_dir is not None else RESULTS_DIR / "a2c"
    ...
    env = DummyVecEnv(
        [
            make_a2c_env(layout_name=layout_name, max_steps=max_steps, seed=seed + index)
            for index in range(num_envs)
        ]
    )
    model = build_a2c_model(
        env=env,
    ...
    print(
        f"Training A2C for {total_timesteps} timesteps on "
    ...
    print(f"Saved A2C model to {final_model_path}")
